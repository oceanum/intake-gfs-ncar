...
class TestReleaseScript:
    ...
    def test_validate_version_valid_versions(self):
        ...
        for version in valid_versions:
            assert release.validate_version(version), f"Version {version} should be valid"
    ...
    def test_validate_version_invalid_versions(self):
        ...
        for version in invalid_versions:
            assert not release.validate_version(version), f"Version {version} should be invalid"
...
''')
    ...
    def test_create_git_tag_dry_run(self, mock_run_command):
        ...
        mock_run_command.assert_called_once_with(["git", "tag", "-l", "v0.3.0"], check=False)
...
class TestReleaseScriptIntegration:
    ...
    def test_main_dry_run_flow(self, mock_create_tag, mock_update_version, 
                              mock_check_branch, mock_check_status, mock_get_version):
