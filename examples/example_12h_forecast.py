...
# Set up logging
log_file = os.path.join(logs_dir, f'gfs_12h_forecast_{datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")}.log')
logging.basicConfig(
    level=logging.INFO,
...
def get_12h_forecast_winds(date_str, model_run='00'):
    ...
    """
    try:
        logger.info(f"Getting 12h forecast wind data for {date_str}, model run {model_run}Z")
        
        # Load the catalog
        catalog_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            'intake_gfs_ncar', 'gfs_catalog.yaml'
        )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Error creating in-memory dataset: {e}", exc_info=True)
                    return None
        ...
        try:
            ...
            try:
                if 'latitude' in ds and 'longitude' in ds:
                    ...
                    logger.warning("Could not find latitude/longitude coordinates for sample data")
