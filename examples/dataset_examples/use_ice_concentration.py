...
def plot_ice_concentration(ds, forecast_step=0, region="north", output_dir="gfs_output"):
    ...
    cs = ax.contourf(lons_cyclic, lats, ci_cyclic, levels=levels,
    ...
    plt.title(f'GFS {title_region} Sea Ice Concentration - {valid_time}\nForecast Step: {step_str}')

    # Save the figure
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join(output_dir, f"gfs_ice_{region}_{timestamp}_{step_str}.png")
...
def main():
    ...
    try:
        ...
        source = cat.gfs_ice_concentration(
            cycle=target_cycle,
            max_lead_time=3  # Get 3 hours of forecast data
        )
        ...
        arctic_plot = plot_ice_concentration(ds, forecast_step=0, region="north", output_dir=output_dir)
        antarctic_plot = plot_ice_concentration(ds, forecast_step=0, region="south", output_dir=output_dir)
        ...
        if 'step' in ds.dims and ds.dims['step'] > 1:
            ...
            for step_idx in range(min(3, ds.dims['step'])):  # Show first 3 steps
                ...
                if ice_mask.sum().values > 0:
                    ...
                    logger.info(f"  Step f{step_val:03d}: No significant ice coverage detected")
        else:
            ...
            if ice_mask.sum().values > 0:
                ...
                logger.info(f"Maximum ice concentration: {float(ds['siconc'].max().values):.2f}")
                logger.info(f"Total area with ice concentration > 0.15: {float(ice_mask.sum().values)} grid cells")
            else:
