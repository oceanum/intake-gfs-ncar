...
def download_file(url, output_path):
    ...
    with urllib.request.urlopen(url, timeout=60) as response, open(output_path, 'wb') as out_file:
    ...
    logger.info(f"Successfully downloaded {os.path.getsize(output_path) / (1024*1024):.2f} MB")
    return output_path

def download_gfs_2m_temperature(date_str, model_run_time=0, forecast_hour=0, output_dir='.'):
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='gfs_') as temp_dir:
            ...
            grib_file = os.path.join(temp_dir, f"gfs.t{model_run_str}z.pgrb2.0p25.f{forecast_str}")
            ...
            try:
                ...
                if not ds.data_vars:
                    logger.warning("No variables found in the dataset. Trying to list available variables...")
...
def main():
    ...
    try:
        ...
        logger.info(f"Starting GFS 2m temperature data download for {date_str} {model_run_time:02d}Z")
        
        # Create an output directory for this run
        output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'gfs_output')
        os.makedirs(output_dir, exist_ok=True)
            ...
            with xr.open_dataset(output_file) as ds:
                ...
                for var_name, var in ds.data_vars.items():
                    ...
                    if var_name not in ds.coords:
                        try:
                            logger.info(f"  Min: {float(var.min().values):.2f} {var.attrs.get('units', '')}")
                            logger.info(f"  Max: {float(var.max().values):.2f} {var.attrs.get('units', '')}")
                        except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        if output_file and os.path.exists(output_file):
            ...
            logger.error("Processing completed with errors. Check the log file for details.")
        ...
        # Make sure to close the source when done
        if 'source' in locals():
            source.close()
